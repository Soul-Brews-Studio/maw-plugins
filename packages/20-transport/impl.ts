import { loadConfig, getTransportRouter } from "maw-js/sdk";

// Transport status — uses the live transport router now that getTransportRouter
// is exposed on the SDK surface (maw-js#646). Prior to #646 this plugin shipped
// a config-only stub because the router was not re-exported. Closes the
// residual standalone-transport half of Soul-Brews-Studio/maw-js#402.
export async function cmdTransportStatus() {
  ...
  const notes: Record<string, string> = {
    "tmux": "local",
    "http-federation": peerCount ? `${peerCount} peer(s)` : "no peers",
    "lora": "no hardware",
  };
